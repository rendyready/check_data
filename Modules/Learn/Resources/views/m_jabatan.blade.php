                ...
                <div class="block-content text-muted">
                    ...
                    <table id="tables" class="table table-bordered table-striped table-vcenter js-dataTable-full">
                        ...
                        <tbody id="tableContents">
                            ...
                            <tr>
                                <td>{{$items->m_level_jabatan_id}}</td>
                                <td>{{$items->m_level_jabatan_nama}}</td>
                            </tr>
                            @endforeach
                        </tbody>
                    </table>
...
<script type="module">
    $(document).ready(function() {
        ...
        $('#tables').Tabledit({
            ...
            url: '{{ route("level-jabatan.action") }}',
            dataType: "json",
            columns: {
                identifier: [0, 'm_level_jabatan_id'],
                editable: [
                    [1, 'm_level_jabatan_nama']
                ]
            },
            restoreButton: false,
            onSuccess: function(data, textStatus, jqXHR) {
                ...
                if (data.action == 'delete') {
                    $('{{route("level-jabatan.action")}}' + data.id).remove();
                }
            }
        });
        $("#tables").append(
