...
class LaporanPembelianController extends Controller
{
    ...
    {
        $data = new \stdClass ();
        $transaksi_rekap = DB::table('rekap_beli')
            ->join('users', 'users_id', 'rekap_beli_created_by')
            ->where('rekap_beli_m_w_id', $request->waroeng);

        if ($request->pengadaan != 'all') {
            $transaksi_rekap->where('rekap_beli_created_by', $request->pengadaan);
        }

        if (strpos($request->tanggal, 'to') !== false) {
            $dates = explode('to', $request->tanggal);
            $transaksi_rekap->whereBetween('rekap_beli_tgl', $dates);
        } else {
            $transaksi_rekap->where('rekap_beli_tgl', $request->tanggal);
        }

        $data->transaksi_rekap2 = $transaksi_rekap->orderBy('rekap_beli_tgl', 'ASC')
